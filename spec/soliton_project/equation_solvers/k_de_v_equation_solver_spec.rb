...
describe SolitonProject::KDeVEquationSolver do
  ...
  before(:each) do
    ...
    @x_interval = 1
    @t_interval = 0.01
    @k_de_v_equation_solver = 
      SolitonProject::KDeVEquationSolver.new(
        @initial_condition,
  ...
  it "should inherit from the EquationSolver class" do
    @k_de_v_equation_solver.should be_a_kind_of(
      SolitonProject::EquationSolver
    )
  end
  
  it "should overwrite the differential_function method" do
    @k_de_v_equation_solver.public_methods(false).should include(
  ...
  describe "#differential_function" do
    ...
    it "should return an object that responds to call" do
      @k_de_v_equation_solver.differential_function.should respond_to(:call)
    end
